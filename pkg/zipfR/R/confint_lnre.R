confint.lnre <- function (object, parm, level=0.95, method=c("normal", "mad", "empirical"), plot=FALSE, ...) {
  ...
  n.parm <- length(parm)
  idx.param <- parm %in% names(object$param)
  idx.gof <- parm %in% names(object$gof)
  idx.S <- parm == "S"
  idx.logP <- parm == "logP"
    idx.fail <- !(idx.param | idx.gof | idx.S | idx.logP)
  ...
  .res.list <- lapply(object$bootstrap, function (.M) {
     ...
     if (any(idx.param)) .res[idx.param] <- sapply(parm[idx.param], function (.name) .M$param[[.name]])
